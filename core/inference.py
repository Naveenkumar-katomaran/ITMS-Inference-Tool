...
class InferenceEngine:
    def __init__(self):
        ...
        # Paths
        vehicle_path = self.config.get("models.vehicle_detection")
        plate_path = self.config.get("models.plate_detection")
        ocr_path = self.config.get("models.ocr")
            ...
            for p_box in plate_results.boxes:
                ...
                if plate_crop.size > 0:
                    ...
                    for o_box in ocr_results.boxes:
                        ...
                        xo1, _, _, _ = o_box.xyxy[0].cpu().numpy()
                        o_conf = o_box.conf[0].cpu().numpy()
                        chars.append((xo1, o_label, o_conf))
                    
                    if chars:
                        chars.sort(key=lambda x: x[0])
                        combined_text = "".join(c[1] for c in chars)
